...
durham %>%
  ggplot(aes(x = age)) + 
  geom_histogram(bins = 10, fill = "#23356D") +
  labs(title = "Distribution of age for Misdemeanor Convictions in Durham County from 1994 to 2019",
       caption = "Dates: 1994-11-01 to 2019-02-04") + 
  theme_minimal()
```
```{r}
...
since2018 <- durham %>%
  ...
  labs(x = "Month",
       ...
       title = "Number of Misdemeanor Convictions per month") + 
  theme_minimal() + 
...
# Misdemeanor convictions over time
